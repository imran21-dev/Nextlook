  ...
  return (
    <div className="h-screen overflow-hidden grid grid-cols-5 justify-items-center p-20 gap-6 relative">
      ...
      <section className="w-full col-span-2">
        <h1 className="logo text-4xl">Nextlook</h1>
        <WordRotate
        ...
        <div className="relative p-10 overflow-hidden bg-transparentbg  w-9/12 mx-auto rounded-2xl">
          ...
          <form onSubmit={handleSignUp}>
            <section className="flex justify-between gap-3">
              <div className="flex flex-col pb-7  w-2/4">
            ...
            <div className="flex flex-col pb-7">
              ...
              <div className="flex items-center bg-transparentbg border border-transparentbg rounded-lg overflow-hidden px-2 gap-2">
                ...
                <input
                  ...
                  className="bg-transparent w-full placeholder:opacity-40 focus:outline-none   py-1 "
                />
                <span onClick={handlePassword} className="opacity-80 hover:bg-transparentbg cursor-pointer duration-150 w-8 h-8 p-2 rounded-full">{showPassword ? <VscEye/> : <VscEyeClosed/>}</span>
              </div>
              <h6 className="text-xs pt-1 opacity-70 font-light">Minimum length is 8 characters</h6>
            </div>
            <button disabled={spin && true} className="flex items-center cursor-pointer justify-center gap-2 w-full rounded-lg py-2 hover:bg-primary bg-primary-deep text-background font-semibold md:text-sm  text-xs">{spin && <ClockLoader size={18}/>} Sign Up</button>
          </form>
          <h2 className="text-center md:text-sm text-xs opacity-70 font-light pt-5">By creating an account, you agree to the <Link href='/login' className="text-primary-deep font-semibold hover:underline">Terms of Service</Link> We&apos;ll accasionaly sen you account related emails.</h2>
          <h2 className="text-center md:text-sm text-xs opacity-80 font-light pt-5">Already have an account? <Link href='/login' className="text-primary-deep font-normal hover:underline">Login</Link></h2>
          <BorderBeam
            duration={10}
            size={200}
